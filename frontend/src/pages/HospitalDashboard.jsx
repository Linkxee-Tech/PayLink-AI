...
import { Search, User, Activity, AlertCircle, CheckCircle, Clock, Wallet, MapPin } from 'lucide-react';
    ...
    <div className="space-y-10">
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          <h1 className="text-4xl font-black text-black">Hospital Console</h1>
          <div className="flex items-center gap-2 text-slate-500 font-bold uppercase text-[10px] tracking-widest bg-white w-fit px-3 py-1 rounded-full border border-slate-100 shadow-sm">
             <MapPin size={12} className="text-primary-500" />
             {JSON.parse(localStorage.getItem('paylink_user'))?.state} <span className="text-slate-300 mx-1">•</span> {JSON.parse(localStorage.getItem('paylink_user'))?.lga}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="card border-primary-100 bg-white p-8">
            ...
            <form onSubmit={handleSearch} className="flex gap-4">
              <input 
                ...
                required
              />
              <button disabled={loading} className="btn-primary px-8 whitespace-nowrap">
          ...
          <AnimatePresence>
            {patient && (
              <motion.div 
                ...
                exit={{ opacity: 0, y: -20 }}
                className="card border-green-100 bg-green-50/30 p-8 space-y-8"
              >
                <div className="flex items-start justify-between">
                  <div className="flex gap-6">
                    <div className="bg-green-100 w-16 h-16 rounded-2xl flex items-center justify-center text-green-600">
                      <User size={32} />
                    </div>
                    <div>
                      <h3 className="text-2xl font-black text-slate-900">{patient.fullName}</h3>
                      <p className="font-mono text-green-600 font-bold">{patient.umhn}</p>
                    </div>
                  </div>
        ...
        <div className="space-y-8">
           <div className="card bg-primary-900 text-white p-8 relative overflow-hidden group">
              ...
              <h3 className="text-primary-300 font-bold uppercase text-[10px] tracking-widest mb-4">Total Life-Saving Recoveries</h3>
              <p className="text-5xl font-black mb-2 tracking-tighter">₦{claims.reduce((acc, c) => acc + c.amount, 0).toLocaleString()}</p>
              <p className="text-primary-400/60 text-xs font-medium">Verified Federal Disbursements</p>
           </div>
